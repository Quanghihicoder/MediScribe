...
docker buildx build --platform linux/amd64 -f Dockerfile.lambda-build -t mediscribe-msk-topic-creator-lambda-builder --load .
...
for /f "tokens=*" %%i in ('docker create mediscribe-msk-topic-creator-lambda-builder') do set CONTAINER_ID=%%i
